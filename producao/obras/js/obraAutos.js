...
fetch('dados/obraAutos.php?codigoProcesso=' + codigoProcesso)
    ...
    .then(data => {
        ...
        // Display the updated JavaScript array
        console.log(obraAutos);
        ...
        obraAutos.forEach((resultado) => {
          ...
          if(resultado["auto_realizado"] != 0 && resultado["auto_previsto"] != 0 ){
              realizado = (resultado["auto_realizado"] / resultado["auto_previsto"]) * 100;
          } else if(resultado["auto_previsto"] == 0 || resultado["auto_previsto"] == null ) {
              realizado = resultado["auto_realizado"] * 100;
          } else {
              realizado = 0;
          };
        ...
        var obraAutosCartoes = `     
            <div onclick="obraAuto('${resultado["auto_num"]}')" class="card col-md-2 ${classeCartao}">
                <div class="d-flex justify-content-between px-md-1">
                    <div class="text-end">
                        <p class="mb-0 small text-white">${resultado["auto_fatura"]} de ${resultado["auto_data"].toLocaleString('pt')} do auto ${resultado["auto_num"]}</p>
                        <!--Faturado-->
