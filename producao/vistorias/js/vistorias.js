...
// Guarda a lista completa vinda do servidor
let listas = {
    vencidas: [],
    programadas: [],
    agendadas: []
...
// NOVO: Guarda os dados que estão atualmente visíveis/filtrados no Dashboard
let dadosAtuais = {
    vencidas: [],
    programadas: [],
    agendadas: []
...
function actualizarDashboard(){
    // Atualiza a variável global dos dados ativos com a totalidade das listas
    dadosAtuais = {
        vencidas: [...listas.vencidas],
        programadas: [...listas.programadas],
        agendadas: [...listas.agendadas]
    };
    renderizarTudo(dadosAtuais);
}
...
function filtrar() {
    ...
    // Atualiza os dadosAtuais apenas com os registos filtrados
    dadosAtuais = {
                ...
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        <small class="text-muted">
                            ...
                            <i class="fa fa-calendar"></i> ${item.doc_num ?? "-"}
                        </small>
                    </div>
                    <div>
...
function actualizarResumo(dados){
    ...
    const valor = [...dados.vencidas, ...dados.programadas, ...dados.agendadas]
...
/* ============================================================
   BOTÕES PDF (Agora usam dadosAtuais)
============================================================ */
function inicializarExportacaoPDF() {
    const pdfVencidas = document.getElementById("pdfVencidas");
    if (pdfVencidas) {
        pdfVencidas.addEventListener("click", () => { exportarPDF(dadosAtuais.vencidas, "Vistorias_Vencidas"); });
    }

    const pdfProgramadas = document.getElementById("pdfProgramadas");
    if (pdfProgramadas) {
        pdfProgramadas.addEventListener("click", () => { exportarPDF(dadosAtuais.programadas, "Vistorias_Programadas"); });
    }

    const pdfAgendadas = document.getElementById("pdfAgendadas");
    if (pdfAgendadas) {
        pdfAgendadas.addEventListener("click", () => { exportarPDF(dadosAtuais.agendadas, "Vistorias_Agendadas"); });
    }
}
...
/* ============================================================
   LIGAÇÃO DOS BOTÕES EXCEL (Agora usam dadosAtuais)
============================================================ */
function inicializarExportacaoExcel() {
    const btn1 = document.getElementById("xlsVencidas");
    if (btn1) {
        btn1.addEventListener("click", () => { exportarExcel(dadosAtuais.vencidas, "Vistorias_Vencidas"); });
    }

    const btn2 = document.getElementById("xlsProgramadas");
    if (btn2) {
        btn2.addEventListener("click", () => { exportarExcel(dadosAtuais.programadas, "Vistorias_Programadas"); });
    }

    const btn3 = document.getElementById("xlsAgendadas");
    if (btn3) {
        btn3.addEventListener("click", () => { exportarExcel(dadosAtuais.agendadas, "Vistorias_Agendadas"); });
    }
}

/* ============================================================
   REDIRECT PROCESSO & AUXILIARES
============================================================ */
